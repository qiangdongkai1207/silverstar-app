<template>
	<view :class="bg+'-bg-gradual'" style="height: 100%;box-sizing: border-box;" :style="{paddingTop:height+'px'}">
		<!--header-->
		<!-- <view class="tui-header-box" :style="{height:height+'px'}">
			<view class="tui-header" :style="{paddingTop:top+'px', opacity:opcity}">
				<view class="tui-header-text icon iconfont icon-clear" @tap="bank()"></view>
			</view>
		</view> -->

		<view class="logo">
			<view class="img">
				<image mode="widthFix"
					src="data:image/png;base64,">
				</image>
			</view>
		</view>
		<!-- 账号密码输入框 -->
		<view class="form">
			<!-- #ifndef MP-WEIXIN -->
			<view class="username">
				<input placeholder="请输入账号" v-model="username" maxlength="20"
					placeholder-style="color: rgba(255,255,255,0.8);" />
			</view>
			<view class="password">
				<input placeholder="请输入密码" type="password" v-model="password" maxlength="20"
					placeholder-style="color: rgba(255,255,255,0.8);" />
			</view>
			<view class="btn" :class="bg+'-bg-white-btn'" @tap="login(1)">登 录</view>
			<view class="res">
				<view @tap="login(2)">用户注册</view>
				<view @tap="login(3)">忘记密码</view>
			</view>
			<!-- #endif -->
			<!-- #ifdef MP-WEIXIN -->
			<button type="primary" open-type="getUserInfo" @getuserinfo="getuserinfoWX" withCredentials="true"
				class="btn" :class="bg+'-bg-white-btn'">一键微信登录</button>
			<!-- #endif -->
		</view>
		<!-- 第三方登录 -->
		<!-- <view class="oauth">
			<view class="text">— 快速登录 —</view>
			<view class="list">
				<view @tap="login(4)" class="icon iconfont icon-tubiaozhizuomoban"></view>
				<view @tap="login(5)" class="icon iconfont icon-wechat"></view>
				<view @tap="login(6)" class="icon iconfont icon-qq"></view>
			</view>
		</view> -->
	</view>
</template>

<script>
	export default {
		data() {
			return {
				// 样式数据 开始
				height: 64, //header高度
				top: 0, //标题图标距离顶部距离
				scrollH: 0, //滚动总高度
				opcity: 1,
				// 样式数据 结束
			}
		},
		computed: {
			bg: {
				get: function() {
					return this.$store.getters.getBg;
				},
				set: function(v) {}
			},
		},
		onLoad() {
			this.initCssHtml();
		},
		onShow() {

		},
		onUnload: function() {},
		onHide: function() {},
		methods: {
			/**
			 * 初始化加载样式
			 */
			initCssHtml() {
				let obj = {};
				// #ifdef MP-WEIXIN
				obj = wx.getMenuButtonBoundingClientRect();
				// #endif
				// #ifdef MP-BAIDU
				obj = swan.getMenuButtonBoundingClientRect();
				// #endif
				// #ifdef MP-ALIPAY
				my.hideAddToDesktopMenu();
				// #endif
				uni.getSystemInfo({
					success: (res) => {
						this.width = obj.left || res.windowWidth;
						this.height = obj.top ? (obj.top + obj.height + 13) : (res.statusBarHeight + 49);
						this.top = (obj.top ? (obj.top + (obj.height - 32) / 2) : (res.statusBarHeight + 6));
						this.scrollH = res.windowWidth * 0.6
						// #ifdef APP-PLUS
						this.top = this.top + 5;
						// #endif
					}
				})
			},
		}
	}
</script>

<style lang="scss">
	@import "login2.scss";
	@import "@/utils/css/tui-header.css";
</style>
